...
describe('computeStyleAdjustment — weighting: logged vs. saved', () => {
  it('applies exactly 3x the delta magnitude for a logged event vs. a saved event, all else equal', () => {
    const currentVector = vector({ adventure: 5 });
    const experienceVector = vector({ adventure: 9 });
    ...
    expect(loggedResult.delta.adventure).toBeCloseTo(savedResult.delta.adventure * 3);
  });
});
...
describe('computeStyleAdjustment — recency decay', () => {
  it('produces a smaller delta the further the event is from the last baseline reset', () => {
    ...
    expect(Math.abs(distantEvent.delta.adventure)).toBeLessThan(
      Math.abs(recentEvent.delta.adventure),
    );
    expect(Math.abs(distantEvent.delta.adventure)).toBeGreaterThan(0);
  });
  ...
  it('applies full weight (decayFactor = 1) when the event is exactly at the baseline date', () => {
    ...
    expect(result.delta.adventure).toBeCloseTo(4);
  });
});
...
describe('computeStyleAdjustment — per-step cap', () => {
  it('clamps a large raw delta to +/- MAX_STEP', () => {
    ...
    const currentVector = vector({ adventure: 0 });
    const experienceVector = vector({ adventure: 10 }); // huge raw gap
    const now = new Date('2026-01-01T00:00:00Z');
    ...
    expect(result.delta.adventure).toBe(0.5);
    expect(result.travelStyle.adventure).toBe(0.5);
  });

  it('clamps a large negative raw delta to -MAX_STEP', () => {
    const cappedConfig: DecayConfig = { wLogged: 3, wSaved: 1, decayLambda: 0, maxStep: 0.5 };
    const currentVector = vector({ adventure: 10 });
    const experienceVector = vector({ adventure: 0 });
    const now = new Date('2026-01-01T00:00:00Z');
    ...
    expect(result.delta.adventure).toBe(-0.5);
    expect(result.travelStyle.adventure).toBe(9.5);
  });

  it('never causes a single event to swing a category by more than MAX_STEP even across all 8 categories', () => {
    const cappedConfig: DecayConfig = { wLogged: 3, wSaved: 1, decayLambda: 0, maxStep: 0.5 };
...
describe('applyManualStyleEdit — baseline reset', () => {
  it('sets travelStyle and travelStyleBaseline to the manually-entered vector, and resets travelStyleLastUpdated', () => {
    const manualVector = vector({ adventure: 7, foodie: 6 });
  ...
  it('causes the next automatic adjustment to decay from the new baseline date, not any prior activity', () => {
    const manualVector = vector({ adventure: 3 });
    ...
    expect(result.delta.adventure).toBeCloseTo(expectedDelta);
  });
});
